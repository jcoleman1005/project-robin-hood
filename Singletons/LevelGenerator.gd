...
@export var camera_boundary_scene: PackedScene
...
# ... (_ready, _process, _draw are the same) ...
func _ready():
	_load_all_chunk_data()
func _process(_delta: float) -> void:
	if Loggie and Loggie.settings.show_chunk_boundaries and is_instance_valid(_level_root):
		queue_redraw()
func _draw() -> void:
	if not (Loggie and Loggie.settings.show_chunk_boundaries and is_instance_valid(_level_root)):
		return
...
func generate_level() -> Node2D:
	...
	Loggie.info("--- Starting Level Generation (Length: %d) ---" % length, "proc_gen")
	...
	for i in range(length):
		...
		if not is_instance_valid(next_chunk.instance) or next_chunk.instance == last_chunk.instance:
			Loggie.info("... Falling back to TRAVERSAL chunk.", "proc_gen")
...
func _add_random_chunk(parent: Node, category: int, previous_chunk: Dictionary, chunks_since_last_dash: int) -> Dictionary:
	...
	if compatible_chunks.is_empty():
		Loggie.warn("No compatible chunks found for category: " + ChunkData.Category.keys()[category], "proc_gen")
	...
	if is_instance_valid(previous_chunk.instance):
		...
		if not is_instance_valid(entry_marker):
			Loggie.error("Chunk " + scene_path + " is missing an 'entry' marker.", "proc_gen")
		...
		Loggie.info(align_log, "proc_gen")
...
func _add_camera_boundary(level_node: Node2D, bounds: Rect2) -> void:
	...
	Loggie.info("Added CameraBoundary at %s with size %s" % [padded_bounds.position.round(), padded_bounds.size.round()], "proc_gen")
...
func _add_killzone(level_node: Node2D, bounds: Rect2) -> void:
	...
	Loggie.info("Added KillZone below level.", "proc_gen")
