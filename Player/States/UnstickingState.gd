...
func process_physics(delta: float) -> void:
	...
	player.global_position -= last_dash_direction * player.stats.core_speed * delta

	if not player.test_move(player.transform, Vector2.ZERO):
		player.velocity = -last_dash_direction * player.stats.core_speed * player.stats.dash_end_velocity_multiplier
